...
const handleOrder = async () => {
  ...
  if (loading) return <p className="text-center mt-10 text-[color:var(--foreground)]">Loading...</p>;
  if (!material) return <p className="text-center text-red-500">Material not found.</p>;
    ...
    <section className="min-h-screen bg-[color:var(--background)] text-[color:var(--foreground)] px-6 py-14 md:px-20">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-[color:var(--muted-foreground)] font-medium text-base">{material.category}</p>

          <div className="space-y-1 mt-2 text-base leading-6">
            <p>💰 <strong>₹{material.pricePerKg}</strong> per kg</p>
            <p>📦 Stock: <strong>{material.availableQuantity} kg</strong></p>
            <p>🏷️ Grade: <strong>{material.qualityGrade}</strong></p>
            <p>📍 Location: <strong>{material.location}</strong></p>
            <p>🚚 Estimated Delivery: <strong>3–5 days</strong></p>
            <p>⭐ Rating: <strong>{material.rating || "4.2"}</strong> / 5</p>
          </div>
          ...
          <div className="bg-[color:var(--muted)] p-4 rounded-xl border border-[color:var(--border)] mt-4">
            ...
            <p className="text-sm text-[color:var(--muted-foreground)]">
              {material.supplierId?.name} ({material.supplierId?.company || "Certified Supplier"})<br />
          ...
          {material.description && (
            <div className="mt-4">
              ...
              <p className="text-sm text-[color:var(--muted-foreground)]">{material.description}</p>
            </div>
          )}
